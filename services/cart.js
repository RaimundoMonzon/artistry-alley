...
export class CartService {
    ...
    async addItem({ id, newItem, quantity }) {
        ...
        const itemAlreadyInCart = cart.items.find(item => item._id.toString() === newItem._id.toString());
        ...
        if (!itemAlreadyInCart) {
            ...
            return await this.saveCart(cart, "Error al agregar el artwork al carrito");
        } else {
    ...
    async updateItemQuantity({ id, itemId, quantity }) {
        const cart = await this.getById(id);
        const itemAlreadyInCart = cart.items.find(item => item._id.toString() === itemId.toString());
        ...
        return await this.saveCart(cart, "Error al actualizar el artwork en el carrito");
    }
    ...
    async removeItem({ id, itemId }) {
        ...
        return await this.saveCart(cart, "Error al eliminar el artwork del carrito");
    }
